 ...
 */
var SHEET_NAME = '변경사항';
var HEADERS = ['id','상태','학원명','시설종류','발송상태','주소','운영자','전화번호','전화번호2','보유차량수','차량형태','차량상세','상담이력','통화결과','갱신시각'];
...
function doGet(e) {
  ...
  for (var i = 0; i < vals.length; i++) {
    ...
    rows.push({
      ...
      carT: g(row, '차량형태'), carDetail: g(row, '차량상세'), consult: g(row, '상담이력'), callres: g(row, '통화결과')
    });
  }
