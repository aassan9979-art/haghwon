 ...
 */
var SHEET_NAME = '변경사항';
var HEADERS = ['id','상태','학원명','주소','운영자','전화번호','전화번호2','보유차량수','차량형태','차량상세','갱신시각'];
...
function doGet(e) {
  ...
  for (var i = 0; i < vals.length; i++) {
    ...
    if (!id) continue;
    rows.push({
      id: String(id), status: g(row, '상태'), name: g(row, '학원명'), addr: g(row, '주소'),
...
function doPost(e) {
  ...
  try {
    ...
    set('id', d.id); set('상태', d.status || ''); set('학원명', d.name || ''); set('주소', d.addr || '');
